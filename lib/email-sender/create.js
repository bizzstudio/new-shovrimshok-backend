...
function generateInvoiceTable(doc, invoice) {
  ...
  for (i = 0; i < invoice.cart.length; i++) {
    ...
    generateTableRow(
      ...
      '',
      item.quantity,
      formatCurrency(invoice.company_info.currency, item.finalPriceAtPurchase.perUnit),
      formatCurrency(invoice.company_info.currency, item.finalPriceAtPurchase.total)
    );
