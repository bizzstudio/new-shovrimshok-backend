...
const addGlobalSetting = async (req, res) => {
  try {
    ...
    console.log('addGlobalSetting error: ', err);
    res.status(500).send({
      message: err.message,
    });
  }
...
const updateGlobalSetting = async (req, res) => {
  try {
    ...
    console.log('updateGlobalSetting error: ', err);
    res.status(500).send({
      message: err.message,
    });
  }
...
const addStoreSetting = async (req, res) => {
  try {
    ...
    console.log('addStoreSetting error: ', err);
    res.status(500).send({
      message: err.message,
    });
  }
...
const getStoreSetting = async (req, res) => {
  try {
    ...
    console.log('getStoreSetting error: ', err);
    res.status(500).send({
      message: err.message,
    });
  }
  ...
  try {
    const storeSetting = await Setting.updateOne(
      ...
      },
      {
        $set: {
          // כיבוי משלוחים
          "setting.delivery_status": req.body.setting.delivery_status,

          // כיבוי הזמנות
          "setting.optionToOrder_status": req.body.setting.optionToOrder_status,
    ...
    console.log('updateStoreSetting error: ', err);
    res.status(500).send({
      message: err.message,
    });
  }
...
const addStoreCustomizationSetting = async (req, res) => {
  try {
    ...
    console.log('addStoreCustomizationSetting error: ', err);
    res.status(500).send({
      message: err.message,
    });
  }
...
const getStoreCustomizationSetting = async (req, res) => {
  ...
  try {
    ...
    console.log('getStoreCustomizationSetting error: ', err);
    res.status(500).send({
      message: err.message,
    });
  }
...
const getStoreSeoSetting = async (req, res) => {
  ...
  try {
    ...
    const defaultSeo = storeCustomizationSetting?.setting?.seo || {};
    const path = req.query.path || '/';

    // פירוש ה-path
    let seoData = {
      favicon: defaultSeo.favicon, // favicon נשאר קבוע
      meta_description: defaultSeo.meta_description,
      meta_img: defaultSeo.meta_img,
      meta_keywords: defaultSeo.meta_keywords,
      meta_title: defaultSeo.meta_title,
      meta_url: defaultSeo.meta_url,
    };
    ...
    if (path.startsWith('/product/')) {
      ...
      if (product) {
        const productTitle = product.title?.he || product.title?.en || '';
        const productDescription = product.description?.he || product.description?.en || '';
        const productImage = product.image && product.image.length > 0 ? product.image[0] : defaultSeo.meta_img;
        
        seoData.meta_title = `${productTitle} | ${process.env.COMPANY_NAME}`;
        seoData.meta_description = productDescription || defaultSeo.meta_description;
        seoData.meta_img = productImage;
        seoData.meta_url = `${defaultSeo.meta_url}product/${slug}`;
      }
    }
    // בדיקה אם זה עמוד קטגוריה
    else if (path.startsWith('/product-category/')) {
      const categoryPath = decodeURIComponent(path.replace('/product-category/', ''));
      const pathParts = categoryPath.split('/').filter(part => part.length > 0);
      
      // אם יש שני חלקים - זה תת קטגוריה
      if (pathParts.length === 2) {
        const parentSlug = pathParts[0];
        const childSlug = pathParts[1];
        
        // טעינת התת-קטגוריה
        const subCategory = await Category.findOne({ slug: childSlug });
        
        if (subCategory) {
          // טעינת הקטגוריה הראשית לפי slug
          const parentCategory = await Category.findOne({ slug: parentSlug });
          
          const subCategoryName = subCategory.name?.he || subCategory.name?.en || '';
          const parentCategoryName = parentCategory?.name?.he || parentCategory?.name?.en || subCategory.parentName || '';
          // עבור תת-קטגוריה, נשתמש בתמונה של הקטגוריה הראשית
          const categoryImage = parentCategory?.icon || parentCategory?.coloredIcon || defaultSeo.meta_img;
          
          seoData.meta_title = `${subCategoryName} - ${parentCategoryName} | ${process.env.COMPANY_NAME}`;
          seoData.meta_description = subCategory.description?.he || subCategory.description?.en || defaultSeo.meta_description;
          seoData.meta_img = categoryImage;
          seoData.meta_url = `${defaultSeo.meta_url}product-category/${encodeURIComponent(parentSlug)}/${encodeURIComponent(childSlug)}`;
        }
      }
      // אם יש חלק אחד - זה קטגוריה ראשית
      else if (pathParts.length === 1) {
        const categorySlug = pathParts[0];
        const category = await Category.findOne({ slug: categorySlug });
        
        if (category) {
          const categoryName = category.name?.he || category.name?.en || '';
          // עבור קטגוריה ראשית, נשתמש בתמונה של הקטגוריה עצמה
          const categoryImage = category.icon || category.coloredIcon || defaultSeo.meta_img;
          
          seoData.meta_title = `${categoryName} | ${process.env.COMPANY_NAME}`;
          seoData.meta_description = category.description?.he || category.description?.en || defaultSeo.meta_description;
          seoData.meta_img = categoryImage;
          seoData.meta_url = `${defaultSeo.meta_url}product-category/${encodeURIComponent(categorySlug)}`;
        }
      }
    }

    // console.log('storeCustomizationSetting?.setting :>> ', { seo: seoData });
    res.send({ seo: seoData });
  } catch (err) {
    console.log('getStoreSeoSetting error: ', err);
    res.status(500).send({
      message: err.message,
    });
  }
...
const updateStoreCustomizationSetting = async (req, res) => {
  try {
    ...
    // קודם נחפש ונדפיס את ההגדרות הקיימות
    const existingSetting = await Setting.findOne({
      name: "storeCustomizationSetting",
    });

    console.log('existing settings: ', existingSetting?.setting);

    // עכשיו נבצע את העדכון
    const storeCustomizationSetting = await Setting.findOneAndUpdate(
      ...
      },
      {
        $set: {
          ...
          // home
          "setting.home.logos_carousel_status": setting.home.logos_carousel_status,
          "setting.home.logos_carousel": setting.home.logos_carousel || [],
          "setting.home.small_banner_img": setting.home.small_banner_img,
          "setting.home.coupon_status": setting.home.coupon_status,
          "setting.home.featured_status": setting.home.featured_status,
          "setting.home.discount_status": setting.home.discount_status,
          ...
          "setting.home.small_banner_link": setting.home.small_banner_link,
          ...
          "setting.about_us.content_middle_status":
            setting.about_us.content_middle_status,
          "setting.about_us.founder_status": setting.about_us.founder_status,
          ...
          "setting.footer.block5_status": setting.footer.block5_status,
          ...
          "setting.footer.block2_sub_title5": setting.footer.block2_sub_title5,
          "setting.footer.block2_sub_link5": setting.footer.block2_sub_link5,
          "setting.footer.block2_sub_title6": setting.footer.block2_sub_title6,
          "setting.footer.block2_sub_link6": setting.footer.block2_sub_link6,
          "setting.footer.block2_sub_title7": setting.footer.block2_sub_title7,
          "setting.footer.block2_sub_link7": setting.footer.block2_sub_link7,
          ...
          "setting.footer.block3_sub_title5": setting.footer.block3_sub_title5,
          "setting.footer.block3_sub_link5": setting.footer.block3_sub_link5,
          "setting.footer.block3_sub_title6": setting.footer.block3_sub_title6,
          "setting.footer.block3_sub_link6": setting.footer.block3_sub_link6,
          "setting.footer.block3_sub_title7": setting.footer.block3_sub_title7,
          "setting.footer.block3_sub_link7": setting.footer.block3_sub_link7,
          "setting.footer.block5_title": setting.footer.block5_title,
          "setting.footer.block5_sub_title1": setting.footer.block5_sub_title1,
          "setting.footer.block5_sub_link1": setting.footer.block5_sub_link1,
          "setting.footer.block5_sub_title2": setting.footer.block5_sub_title2,
          "setting.footer.block5_sub_link2": setting.footer.block5_sub_link2,
          "setting.footer.block5_sub_title3": setting.footer.block5_sub_title3,
          "setting.footer.block5_sub_link3": setting.footer.block5_sub_link3,
          "setting.footer.block5_sub_title4": setting.footer.block5_sub_title4,
          "setting.footer.block5_sub_link4": setting.footer.block5_sub_link4,
          "setting.footer.block5_sub_title5": setting.footer.block5_sub_title5,
          "setting.footer.block5_sub_link5": setting.footer.block5_sub_link5,
    ...
    console.log('updateStoreCustomizationSetting error: ', err);
    res.status(500).send({
      message: err.message,
    });
  }
...
const restoreStoreCustomizationSetting = async () => {
  try {
    const backupPath = path.join(__dirname, "../data/settingsBackup.json");
    const backupData = JSON.parse(fs.readFileSync(backupPath, "utf8"));

    await Setting.findOneAndUpdate(
      { name: "storeCustomizationSetting" },
      { $set: { name: "storeCustomizationSetting", setting: backupData } },
      { new: true, upsert: true }
    );

    console.log("storeCustomizationSetting restored from settingsBackup.json");
  } catch (err) {
    console.log("restoreStoreCustomizationSetting error: ", err);
  }
};

// setTimeout(() => {
//   restoreStoreCustomizationSetting();
// }, 10000);

//store scripts setting controller
const getStoreScriptsSetting = async (req, res) => {
  try {
    const scripts = await Setting.findOne({ name: "storeScriptsSetting" }, { setting: 1, _id: 0 });
    res.send(scripts?.setting || { head: "", bodyStart: "", bodyEnd: "" });
  } catch (err) {
    console.log('getStoreScriptsSetting error: ', err);
    res.status(500).send({
      message: err.message,
    });
  }
...
const updateStoreScriptsSetting = async (req, res) => {
  try {
    const { head = "", bodyStart = "", bodyEnd = "" } = req.body?.setting || {};

    const updated = await Setting.findOneAndUpdate(
      { name: "storeScriptsSetting" },
      {
        $set: {
          "setting.head": head,
          "setting.bodyStart": bodyStart,
          "setting.bodyEnd": bodyEnd,
          name: "storeScriptsSetting",
        },
      },
      { new: true, upsert: true }
    ...
    res.send({
      ...
      message: {
        he: "הסקריפטים בחנות עודכנו בהצלחה!",
        en: "Store Scripts Updated Successfully!",
      },
    });
  } catch (err) {
    console.log('updateStoreScriptsSetting error: ', err);
    res.status(500).send({
      message: err.message,
    });
  }
